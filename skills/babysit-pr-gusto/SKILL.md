...
- **Ignore manual-review-only checks** — Chromatic ("UI Tests"/"UI Review") gates on a human accepting visual diffs; the babysitter can neither fix nor wait on it. `triage-prs.sh` excludes it (configurable via `BABYSIT_IGNORE_CHECKS`) so a PR green on every other check triages "green" and is babysitter-done, while the pending Chromatic check is surfaced in `ignored` — report it as "manual review pending (human)" so the human approves + merges. Never treat it as a babysitter failure, and never try to "fix" it.
