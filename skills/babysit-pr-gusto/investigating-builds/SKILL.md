...
| ----------------- | --------------- | ---------- | --------- |
| Parse any BK URL  | yes             | no         | no        |
| Get build details | yes             | yes        | yes       |
| Get annotations   | yes             | yes        | yes       |
| **Retrieve logs** | **yes**         | no         | yes       |
| Save to files     | yes             | no         | no        |
| Wait for build    | no              | no         | yes       |
| Unblock jobs      | no              | no         | yes       |
...
```bash
${CLAUDE_SKILL_DIR}/investigating-builds/scripts/wait-for-build.js <org> <pipeline> <build-number> --timeout 1800 --interval 30
```
...
**Why**: Familiarity does not equal effectiveness. You'll spend more time working around GitHub's limitations than learning the proper tools.
