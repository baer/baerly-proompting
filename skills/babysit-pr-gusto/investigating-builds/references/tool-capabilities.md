...
| --------------------- | ------------------- | ------------------- | ------------------------------- | ------------------------------ |
| Parse any BK URL      | yes                 | no                  | no                              | snapshot handles URL parsing   |
| List pipelines        | no                  | yes `pipelines`     | yes `buildkite:list_pipelines`  |                                |
| List builds           | no                  | yes `builds`        | yes `buildkite:list_builds`     |                                |
| Get build details     | yes (build.json)    | yes `build`         | yes `buildkite:get_build`       |                                |
| Get annotations       | yes (annotations.json) | yes `annotations` | yes `buildkite:list_annotations` |                               |
| **Retrieve job logs** | **yes** (steps/*/log.txt) | **no**         | **yes `buildkite:get_logs`**    | snapshot captures automatically |
| Save to files         | yes                 | no                  | no                              | For later analysis             |
| List artifacts        | no                  | no                  | yes `buildkite:list_artifacts`  |                                |
| Wait for build        | no                  | no                  | yes `buildkite:wait_for_build`  |                                |
| Unblock jobs          | no                  | no                  | yes `buildkite:unblock_job`     |                                |
| Human-readable output | yes                 | yes                 | no (JSON)                       |                                |
...
**Access Method:** `${CLAUDE_SKILL_DIR}/investigating-builds/scripts/<script>.js`
...
```bash
${CLAUDE_SKILL_DIR}/investigating-builds/scripts/find-commit-builds.js <org> <commit-sha>
```
...
```bash
${CLAUDE_SKILL_DIR}/investigating-builds/scripts/wait-for-build.js <org> <pipeline> <build> [options]
```
...
```bash
${CLAUDE_SKILL_DIR}/investigating-builds/scripts/get-build-logs.js <org> <pipeline> <build> <job-label-or-uuid>
```
...
**Why**: Step IDs are not Job UUIDs. MCP tools need job UUIDs.
